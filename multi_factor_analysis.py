...
# 导入回测工具类
from backtest_utils import DateStrategy, StockTradeAnalyzer, plot_strategy_performance, get_trading_days
...
class MultiFactorCalculator:
    ...
    def __init__(self, name="多因子组合", description="基于多个因子加权组合的股票选择策略"):
        """
        初始化多因子计算器
    ...
    def add_factor(self, factor_name: str, factor_calculator: FactorCalculator, weight: float = 1.0):
        """
        添加因子到多因子组合中
    ...
    def calculate(self, df: pd.DataFrame, factor_params: Optional[Dict[str, Dict]] = None) -> pd.DataFrame:
        """
        计算多因子组合值
    ...
    def _standardize_factors(self, df: pd.DataFrame, factor_columns: List[str]) -> pd.DataFrame:
        """
        标准化因子值（按交易日标准化）
        ...
        for trade_date in df['trade_date'].unique():
            ...
            if valid_mask.sum() > 1:  # 至少需要2个有效样本
                ...
                df_result.loc[date_mask & valid_mask, factor_columns] = standardized_data
        
        return df_result
    
    def _calculate_composite_factor(self, df: pd.DataFrame, factor_columns: List[str]) -> pd.DataFrame:
        """
        计算复合因子值
    ...
    def analyze_factor_correlation(self, df: pd.DataFrame, trade_date: Optional[str] = None) -> pd.DataFrame:
        """
        分析因子间相关性
...
def run_multi_factor_backtesting(df: pd.DataFrame, all_trade_dates: List[datetime], 
                                multi_factor_calculator: MultiFactorCalculator, 
                                rebalance_period: int, hold_top: int, 
                                factor_params: Optional[Dict[str, Dict]] = None) -> Tuple[set, Dict, Dict]:
    """
    多因子回测数据处理函数
    ...
    for i in range(0, len_all_trade_dates, rebalance_period):
        ...
        # 选择下一交易日可以交易的股票
        valid_stocks = df[(df['trade_date'] == all_trade_dates[i]) & 
                         (df['stock_code'].isin(halted_stocks) == False) &
                         (df[composite_factor_col].notna())]  # 过滤掉复合因子值为空的股票
        
        if not valid_stocks.empty:
            # 复合因子按降序排列（高值优先）
            selected_stocks = valid_stocks.sort_values(
                by=composite_factor_col, 
                ascending=False
            ).head(hold_top)['stock_code'].tolist()
        else:
            selected_stocks = []
...
def plot_factor_analysis(df: pd.DataFrame, multi_factor_calculator: MultiFactorCalculator, 
                        trade_date: str) -> Tuple[go.Figure, go.Figure]:
    """
    绘制因子分析图表
    ...
    """
    # 因子相关性分析
    correlation_matrix = multi_factor_calculator.analyze_factor_correlation(df, trade_date)
    ...
    fig_weights = go.Figure(data=[
        go.Bar(x=factor_names, y=weight_values, 
...
def main():
        ...
        if factor_selected:
            ...
            if factor_name == "市值因子":
                ...
                momentum_period = st.sidebar.slider(
                    "动量回看期（交易日）", 
                    min_value=5, max_value=60, value=21,
    ...
    # 基础策略参数
    rebalance_period = st.sidebar.slider("调仓周期（交易日）", min_value=5, max_value=60, value=21)
    hold_top = st.sidebar.slider("持有股票数量", min_value=5, max_value=30, value=10)
    
    # 因子标准化选项
    standardize_factors = st.sidebar.checkbox("标准化因子值", value=True, help="对因子值进行标准化处理，消除量纲影响")
    ...
    if enable_stop_loss:
        stop_loss_pct = st.sidebar.slider("止损比例 (%)", min_value=1.0, max_value=20.0, value=10.0, step=0.5) / 100
    ...
    with col2:
        ...
        if enable_stop_loss and stop_loss_pct is not None:
            st.write(f"**止损设置：** {stop_loss_pct*100:.1f}% ({'移动' if trailing_stop else '固定'})")
        else:
            st.write("**止损设置：** 未启用")
    
    # 运行回测按钮
    if st.button("🚀 开始多因子回测", type="primary", use_container_width=True):
        # 转换日期格式
        ...
        with st.spinner("正在运行多因子回测..."):
            try:
                ...
                df = findata.get_stock_data_frame_in_sector(sector_name, pre_start_date, end_str, adj='hfq')
                all_trade_dates = get_trading_days(df, start_date)
                
                # 运行多因子回测
                stock_list, buy_dates, sell_dates = run_multi_factor_backtesting(
                    df, all_trade_dates, multi_factor_calculator, rebalance_period, hold_top, factor_params
                )
                ...
                for stock in stock_list:
                    df_stock = df[(df['stock_code'] == stock) & (df['trade_date'].isin(all_trade_dates))].bfill().set_index('trade_date')
                ...
                # 添加策略和分析器
                cerebro.addstrategy(DateStrategy, 
                                  data_source=findata,
                                  buy_dates=buy_dates,
                                  ...
                                  log_file=None)
                cerebro.addanalyzer(StockTradeAnalyzer, _name='stock_trade_analyzer', data_source=findata)
                ...
                chart = plot_strategy_performance(findata, trade_analyzer, pre_start_str, end_str, sector_code)
                ...
                try:
                    ...
                    # 绘制因子分析图表
                    fig_corr, fig_weights = plot_factor_analysis(df_analysis, multi_factor_calculator, analysis_date)
                ...
                with st.expander("📋 详细分析结果"):
                    ...
                    if enable_stop_loss and stop_loss_pct is not None:
                        ...
                        st.write(f"- 止损类型: {'移动止损' if trailing_stop else '固定止损'}")
                    else:
                        st.write("- 止损: 未启用")
                    ...
                    if analysis_result['stock_analysis_df'] is not None and not analysis_result['stock_analysis_df'].empty:
                        st.write("**📈 股票交易分析数据:**")
                        st.dataframe(
