...
def get_trading_days(df, start_date) -> list:
    ...
    idx = next(
        (i for i, d in enumerate(all_trade_dates_full) if d >= start_ts),
        len(all_trade_dates_full),
    )
    if idx > 0:
    ...
    all_trade_dates = [
        date for date in all_trade_dates_full if date >= previous_trading_day
    ]
...
class StockTradeAnalyzer(bt.Analyzer):
    ...
    def notify_order(self, order):
        ...
        source = self.data_source or self.p.data_source
        stock_name = source.get_stock_name(stock_code) if source else "UNKNOWN"
    ...
    def stop(self):
        ...
        for stock_code, trade_record in self.stock_trades.items():
            ...
            print(
                f"  买入次数: {len(trade_record['buy_orders'])}, 买入总额: {total_buy:,.2f} 元"
            )
            print(
                f"  卖出次数: {len(trade_record['sell_orders'])}, 卖出总额: {total_sell:,.2f} 元"
            )
            if is_holding and current_position:
                print(
                    f"  当前持仓市值: {current_market_value:,.2f} 元 (持仓 {current_position.size} 股)"
                )
                print(f"  持仓状态: 🔵 持有中")
            else:
                print(f"  持仓状态: ✅ 已清仓")
...
class DateStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # 确保日志目录存在
        log_dir = (
            os.path.dirname(os.path.abspath(self.log_file))
        ...
        for data in self.datas:
            ...
            if position.size > 0 and data in self.entry_prices:
                ...
                if should_stop:
                    ...
                    stock_name = (
                        self.data_source.get_stock_name(stock_code)
                        if self.data_source
                        else "UNKNOWN"
                    )
                    ...
                    if self.p.trailing_stop:
                        trailing_loss_pct = (
                            current_price / self.highest_prices[data] - 1
                        ) * 100
                        self.log(
                            f"🛑 移动止损: {stock_code}({stock_name}) 买入价{entry_price:.2f} 最高价{self.highest_prices[data]:.2f} 当前价{current_price:.2f} 止损{stop_loss_pct:.2f}% 回撤{trailing_loss_pct:.2f}%"
                        )
                    else:
                        self.log(
                            f"🛑 固定止损: {stock_code}({stock_name}) 买入价{entry_price:.2f} 当前价{current_price:.2f} 止损{stop_loss_pct:.2f}%"
                        )
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                self.log(
                    f"买入{stock_code}({stock_name}), 成交价{order.executed.price:.2f}, 成交量{order.executed.size}, 佣金{order.executed.comm:.2f}"
                )
                ...
                self.log(
                    f"卖出{stock_code}({stock_name}), 成交价{order.executed.price:.2f}, 成交量{abs(order.executed.size)}, 佣金{order.executed.comm:.2f}"
                )
...
def plot_strategy_performance(
    data_source: LocalData, analyzer, start_date, end_date, sector_name="000300.SH"
):
    """
    ...
    try:
        ...
        if not index_df.empty:
            ...
            # 获取指数名称用于显示
            index_name = (
                data_source.get_stock_name(sector_name) if data_source else sector_name
            )
            ...
            line_chart = (
                ...
                .add_yaxis(
                    f"{index_name}收益率(%)",
                    [
                        index_returns[min(i, len(index_returns) - 1)]
                ...
                .set_global_opts(
                    title_opts=opts.TitleOpts(
                        title=f"策略 vs {index_name} - 收益率对比"
                    ),
            ...
            final_index_return = (
                index_df.iloc[-1]["close"] / initial_index_price - 1
            ) * 100
...
def run_date_strategy_backtest(
    ...
    # 添加分析器
    analyzer = cerebro.addanalyzer(
        StockTradeAnalyzer, _name="trade_analyzer", data_source=data_source
    )
