...
class MomentumFactor(FactorCalculator):
    ...
    def calculate(self, df, momentum_period=21, **kwargs):
        ...
        # 按股票分组后分别计算每只股票的动量
        factor_col = self.get_factor_column()
        df[factor_col] = df.groupby('stock_code')['close'].pct_change(periods=momentum_period)
...
class VolatilityFactor(FactorCalculator):
    ...
    def __init__(self):
        super().__init__("波动率", "基于价格波动率进行股票排序，低波动率优先", ascending=True)
...
class TurnoverFactor(FactorCalculator):
    ...
    def __init__(self):
        super().__init__("换手率", "基于换手率进行股票排序，高换手率优先", ascending=False)
...
def run_single_factor_backtesting(df, all_trade_dates, factor_calculator, rebalance_period, 
                                hold_top, factor_params=None):
    """
    通用的单因子回测数据处理函数
    ...
    for i in range(0, len_all_trade_dates, rebalance_period):
        ...
        valid_stocks = df[(df['trade_date'] == all_trade_dates[i]) & 
                         ...
                         (df[factor_col].notna())]  # 过滤掉因子值为空的股票
        
        if not valid_stocks.empty:
            # 根据因子计算器的排序方式进行排序
            selected_stocks = valid_stocks.sort_values(
                by=factor_col, 
                ascending=factor_calculator.ascending
...
def main():
    ...
    if selected_factor_name == "市值因子":
        market_option = st.sidebar.selectbox("市值计算方式", ["总市值", "流通市值"], index=0)
        smooth_window = st.sidebar.slider("市值平滑窗口（交易日）", min_value=1, max_value=20, value=7)
        factor_params = {
        ...
        momentum_period = st.sidebar.slider("动量回看期（交易日）", min_value=5, max_value=60, value=21)
        factor_params = {
        ...
        volatility_period = st.sidebar.slider("波动率计算周期（交易日）", min_value=5, max_value=60, value=20)
        factor_params = {
            "volatility_period": volatility_period
        }
        
    elif selected_factor_name == "换手率因子":
        turnover_period = st.sidebar.slider("换手率平滑周期（交易日）", min_value=5, max_value=60, value=20)
        factor_params = {
        ...
        with st.spinner("正在运行回测..."):
            try:
                ...
                stock_list, buy_dates, sell_dates = run_single_factor_backtesting(
                    df, all_trade_dates, selected_factor, rebalance_period, hold_top, factor_params
                )
